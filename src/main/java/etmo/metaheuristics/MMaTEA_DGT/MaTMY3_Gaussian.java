...
public class MaTMY3_Gaussian extends MtoAlgorithm {
    ...
    void initState() throws JMException, ClassNotFoundException {
        ...
        Arrays.fill(tP, transferProbability);//将数组全部附上一样的值
    ...
    Object generatingOffspring(int taskID) throws JMException {
        ...
        for (int i = 0; i < populationSize; i ++) {
            child = null;
            if (PseudoRandom.randDouble() < tP[taskID]) {//tP[taskID]
...
//        double[] newFeatures = Probability.sampleByNorm(tmpMean, tmpStd);
        ...
        double[] newFeatures;
            ...
            Vector.vecSub_(tmpMean, Vector.vecElemMul(means[assistTaskID], factor));
...
//         mutateIndividual(taskID, child);
    ...
    Solution evolutionaryGenerating(int taskID, int i, String type) throws JMException {
        ...
        }else if(type.equalsIgnoreCase("GS"))
        {
            ...
            int j = PseudoRandom.randInt(0, population[taskID].size() - 1);
            child=new Solution(population[taskID].get(j));

            tmpMean = population[taskID].getMean();
            tmpStd = population[taskID].getStd();
            newFeatures = Probability.sampleByNorm(tmpMean, tmpStd);
            Vector.vecClip_(newFeatures, 0.0, 1.0);
            child.setDecisionVariables(newFeatures);
...
//            double[] scores = new double[taskNum];
        ...
        // EMD
            ...
            //记录选择的任务
//            String filename = "/home/wangqianhui/CEC2021/select_task/"+run_mode+"/example.txt";
//           // String filename="example.txt";
//            String content = res1 + " " + res2;
//            if (res1!=res2)
//                content = content+" 1\n";
//            else
//                content=content+" 0\n";
...
//                writer.close();
//                //System.out.println("Integers have been written to the file.");
//            } catch (IOException e) {
...
//        Arrays.setAll(scores, i -> 1 / distances2[taskID][i]);
//        int res2 = Random.rouletteWheel(scores, taskID);
//       //LS assistTaskID = PseudoRandom.randDouble() < 0.5 ? res1 : res2;
//        assistTaskID = res2 ;
    ...
    public static String readFromFile(String fileName) throws IOException {
        ...
        File file = new File(fileName+Processid);
