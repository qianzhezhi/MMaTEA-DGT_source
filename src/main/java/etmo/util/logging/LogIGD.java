...
public class LogIGD {
    ...
    public static void LogPF(String algoName, double[][] pfs, int task_no, int time) {
        String folderPath = "./data/PFs/_collections/" + algoName;
        File folder = new File(folderPath);
        if (!folder.exists()){
            folder.mkdirs();
        }

        int times = pfs[0].length;
        String filePath = folderPath + "/" + algoName + "_DVA_task_" + task_no + "_time_" + time + ".txt";
        try {
            /* Open the file */
            FileOutputStream fos = new FileOutputStream(filePath);
            OutputStreamWriter osw = new OutputStreamWriter(fos);
