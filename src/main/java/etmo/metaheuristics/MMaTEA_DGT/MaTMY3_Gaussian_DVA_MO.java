...
public class MaTMY3_Gaussian_DVA_MO extends MtoAlgorithm {
...
//    double[][] stds;
    ...
    public MaTMY3_Gaussian_DVA_MO(ProblemSet problemSet) {
        super(problemSet);
    }
...
//                printIGD();
    ...
    void printIGD() {
        ...
        for (int k = 0; k < taskNum; ++k) {
            System.out.println(processIGD[k][generation]);
        }
        System.out.println();
        for (int k = 0; k < taskNum; ++k) {
...
//        // 2
//        double[] tmpMean = means[assistTaskID];
//        Vector.vecAdd_(tmpMean, population[assistTaskID].getMean());
//        Vector.vecSub_(tmpMean, means[taskID]);
    ...
    void environmentSelection() throws ClassNotFoundException, JMException {
        ...
        for (int taskID = 0; taskID < taskNum; taskID ++) {
            ...
            //     System.out.println(generation + ": " + x[taskID]);
            // }
...
//                         transferredEliteCounts[taskID][sf] = 1;
             ...
             tP[taskID] = 0.9 * tP[taskID] + 0.1 * (transferredEliteCount * tP[taskID] / eliteCount);
            // // if (taskID == plotTaskID) {
